...
const investmentGroup = cfoGroups.groups.find((group) => group.key === 'investment');
const pensionGroup = cfoGroups.groups.find((group) => group.key === 'pension');
assert.equal(investmentGroup.assetAmount, 5_000_000);
assert.equal(pensionGroup.assetAmount, 3_000_000);
assert.deepEqual(investmentGroup.items.map((item) => item.id), ['invest']);
assert.deepEqual(pensionGroup.items.map((item) => item.id), ['pension']);
assert.equal(investmentGroup.items.some((item) => item.cfoGroupKey === 'pension'), false);
assert.equal(pensionGroup.items.some((item) => item.cfoGroupKey === 'investment'), false);
