...
const checklist = (await readFile(new URL('../js/features/checklist.js', import.meta.url), 'utf8')).replace(/\r\n/g, '\n');
...
assert.ok(index.includes('aria-label="현금흐름으로 이동"'));
...
assert.ok(appShell.includes("const homeLabel = '홈'"));
...
assert.ok(index.includes('id="cashflow-view" hidden inert'));
...
assert.ok(!checklist.includes('id="checklist-group-input"'), 'Career/Finance/Life/Work are the Todo groups; custom task groups must stay removed');
...
assert.ok(checklist.includes("DOMAINS.map((domain) =>"), 'the left rail must show every fixed Todo domain together');
assert.ok(checklist.includes("{ key: 'work', label: 'Work', tone: 'slate' }"), 'Work must be the fourth fixed Todo domain');
assert.ok(checklist.indexOf("key: 'life'") < checklist.indexOf("key: 'work'"), 'Work must appear after Life');
assert.ok(checklist.includes("|| DOMAINS.find((item) => item.key === 'life')"), 'unknown legacy domains must keep falling back to Life');
...
assert.ok(checklist.includes('applyNoteIndentation(noteSurface.dataset.noteTarget, event.shiftKey)'), 'Shift+Tab must outdent Todo notes');
assert.ok(!checklist.includes('data-checklist-note-command'), 'Todo notes must not expose checklist commands');
assert.ok(!checklist.includes('data-checklist-icon-picker-toggle') && !checklist.includes('data-checklist-note-icon'), 'Todo notes must not expose inline icons');
assert.ok(!checklist.includes('data-note-surface-checkbox'), 'Todo note checkbox syntax must render as ordinary memo text');
...
assert.ok(!checklist.includes('data-checklist-note-block') && !checklist.includes('NOTE_BLOCKS'), 'Todo notes must not expose block features');
assert.ok(!checklist.includes('data-checklist-note-convert'), 'Todo notes must not convert memo text to a Todo or Step');
