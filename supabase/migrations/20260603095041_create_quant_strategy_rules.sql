...
comment on table public.quant_strategy_rules is 'NetVisualizer Quant strategy target weights and rebalance bands.';
comment on column public.quant_strategy_rules.strategy_tag is 'Strategy key shared with portfolios.strategy_tag.';
comment on column public.quant_strategy_rules.target_pct is 'Target portfolio weight percent.';
comment on column public.quant_strategy_rules.band_pct is 'Allowed drift from target percent before rebalance signal.';
comment on column public.quant_strategy_rules.trigger_label is 'Human-readable signal basis for the strategy.';

insert into public.quant_strategy_rules (strategy_tag, target_pct, band_pct, trigger_label, is_active, display_order)
values
