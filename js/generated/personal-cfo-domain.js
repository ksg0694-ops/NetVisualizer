var PersonalCfoDomain = (function(exports) {
	...
	function getSummaryAssetGroupKey(purposeKey, isPension = false) {
	...
	function getAssetSummaryGroups(snapshot) {
		const totals = /* @__PURE__ */ new Map();
		const add = (purposeKey, amount, isPension = false) => {
			if (amount <= 0) return;
		...
		if (snapshot.assets.length > 0) {
			...
			snapshot.assets.forEach((asset) => {
				const account = asset.accountId ? accountById.get(asset.accountId) : void 0;
				add(asset.purposeKey, asset.marketValue, asset.assetClass === "pension" || account?.accountType === "pension");
			});
		} else snapshot.accounts.forEach((account) => add(account.purposeKey, account.balance, account.accountType === "pension"));
		return Object.keys(summaryAssetGroupMeta).map((key) => ({
	...
	function getCashFlowSummaryData(snapshot) {
		...
		const monthlyByPurpose = /* @__PURE__ */ new Map();
	...
	function buildCashFlowGraph(snapshot) {
		...
		if (summary.allocationShortfall > 0) {
			...
			edges.push(makeEdge("edge:shortfall:allocation", "liability:salary-allocation-shortfall", allocationId, "EXPOSED_TO", summary.allocationShortfall));
		}
		[
			{
			...
			edges.push(makeEdge(`edge:allocation:${destination.id}`, allocationId, destination.id, destination.type === "liability" ? "FLOWS_TO" : "ALLOCATED_TO", destination.amount));
		});
	...
	function buildCombinedSummaryGraph(snapshot) {
		const nodes = [];
		const edges = [];
		...
		if (cashFlow.allocationShortfall > 0) {
			...
			edges.push(makeEdge("edge:combined:shortfall:allocation", "liability:salary-allocation-shortfall", allocationId, "EXPOSED_TO", cashFlow.allocationShortfall));
		}
		[
			{
				id: "summary:cashflow:living",
			...
			edges.push(makeEdge(`edge:combined:allocation:${group.id}`, allocationId, group.id, group.type === "liability" ? "FLOWS_TO" : "ALLOCATED_TO", group.amount));
		});
		assetGroups.forEach((group, index) => {
			nodes.push(makeNode({
				id: group.id,
		...
		const flowToAsset = [
			...
			},
			{
				...
				targetKey: "defense",
				amount: cashFlow.purposeAllocations.defense
			},
			{
				source: "summary:cashflow:saving",
				targetKey: "housing",
				amount: cashFlow.purposeAllocations.housing
			},
			{
				source: "summary:cashflow:saving",
				targetKey: "growth",
				amount: cashFlow.purposeAllocations.growth
			},
			{
				source: "summary:cashflow:saving",
				targetKey: "pension",
				amount: cashFlow.purposeAllocations.pension
			},
			{
				source: "summary:cashflow:saving",
				targetKey: "other",
				amount: cashFlow.purposeAllocations.other
			}
		];
		...
		flowToAsset.forEach((connection) => {
			...
			edges.push(makeEdge(`edge:combined:${connection.source}:${target.id}`, connection.source, target.id, "ALLOCATED_TO", connection.amount));
		});
		nodes.push(makeNode({
			id: snapshot.person.id,
			label: snapshot.person.label,
	...
	function buildFinanceGraphFromSnapshot(snapshot, mode = "combined") {
	...
	function createPersonalCfoPageModel(snapshot, graphMode = "combined") {
		return {
			snapshot,
