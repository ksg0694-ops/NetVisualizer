(function(root){
    ...
    function renderChart(){
        ...
        chart?.destroy();
        chart=new root.Chart(el('bs-chart'),{type:mode==='balance'?'line':'bar',data:{labels,datasets},options:{responsive:true,maintainAspectRatio:false,animation:false,interaction:{mode:'index',intersect:false},scales:{x:{grid:{display:false},ticks:{maxRotation:0,maxTicksLimit:7}},y:{beginAtZero:mode==='change',ticks:{callback:v=>`${(v/100000000).toLocaleString('ko-KR',{maximumFractionDigits:4})}억`},grid:{color:'#f1f3f5'}}},plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>`${c.dataset.label}: ${won(c.raw)}`}}}}});
