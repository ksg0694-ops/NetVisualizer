...
- Tab 입력 시 현재 줄에 공백 3개 추가: 통과
- Shift+Tab 입력 시 공백 3개 제거: 통과
- 체크박스 미리보기가 textarea 위에서 즉시 표시: 통과
- 체크박스 크기 12×12 CSS px 및 수직 중앙 정렬: 통과
- 일반 문장과 체크박스 줄바꿈 유지: 통과
- PC 1280×720 및 모바일 390×844 가로 오버플로 없음: 통과
- 생활노트 DOM·스크립트·PWA 캐시 참조 제거: 통과
- 전체 검사 및 브라우저 콘솔 오류 0건: 통과

## 단일 편집면 정정 검사

- 상세내역에 보이는 편집면 1개, 별도 미리보기 0개, 보이는 textarea 0개: 통과
- 렌더링된 텍스트를 직접 입력·선택·수정하고 숨겨진 저장 원문과 자동 동기화: 통과
- 체크박스 추가/완료 상태, Tab 3칸 들여쓰기, Shift+Tab 내어쓰기: 통과
- 굵게/밑줄/취소선 및 아이콘을 같은 편집면에서 적용: 통과
- 기존 `**`, `++`, `~~`, `{{icon:*}}`, `- [ ]` 저장 형식과 호환: 통과
- PC 1280×720 및 모바일 390×844 캡처, 콘솔 오류 0건: 통과
- 모바일 저장 버튼 높이 과확장 제거: 통과
